...
var nicEditorConfig = bkClass.extend({
	...
	iconsPath : STATIC_URL + 'poems/js/nicEditorIcons.gif',
	...
	addForm : function(f,elm) {
		...
		for(itm in f) {
			...
			if(type == 'title') {
				...
				switch(type) {
					case 'text':
						this.inputs[itm] = new bkElement('input').setAttributes({id : itm, 'value' : val, 'type' : 'text'}).setStyle({margin : '2px 0', fontSize : '13px', 'float' : 'left', height : '20px',  overflow : 'hidden'}).setStyle(field.style).appendTo(contain);
						break;
					case 'select':
						this.inputs[itm] = new bkElement('select').setAttributes({id : itm}).setStyle({ 'float' : 'left', margin : '2px 0'}).appendTo(contain);
						for(opt in field.options) {
							var o = new bkElement('option').setAttributes({value : opt, selected : (opt == val) ? 'selected' : ''}).setContent(field.options[opt]).appendTo(this.inputs[itm]);
						}
						break;
					case 'content':
						this.inputs[itm] = new bkElement('textarea').setAttributes({id : itm}).setStyle({ 'float' : 'left'}).setStyle(field.style).appendTo(contain);
						this.inputs[itm].value = val;
				}	
			}
		}
		new bkElement('input').setAttributes({'type' : 'submit'}).setStyle({backgroundColor : '#efefef', margin : '3px 0', 'float' : 'left', 'clear' : 'both'}).appendTo(this.form);
...
var nicUploadButton = nicEditorAdvancedButton.extend({	
	...
	addPane : function() {
		...
		this.progressWrapper = new bkElement('div').setStyle({display: 'none', width: '100%', height: '20px'}).appendTo(this.pane.pane);
